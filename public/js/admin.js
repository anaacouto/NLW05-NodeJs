...
socket.on("admin_receive_message", (data) => {
  const conection = connectionsUsers.find(connection => connection.socket_id === data.socket_id);
  const divMessages = document.getElementById(`allMessages${conection.user_id}`);
  const createDiv = document.createElement("div");
  createDiv.className = "admin_message_client";
  createDiv.innerHTML = `<span>${conection.user.email}</span>`;
